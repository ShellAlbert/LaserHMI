#include "zcircleindicator.h"
#include <QPainter>
ZCircleIndicator::ZCircleIndicator(QString title,QWidget *parent) :
        QWidget(parent)
{
    this->m_title=title;
    this->initVariables();
...
void ZCircleIndicator::initVariables()
{
...
void ZCircleIndicator::paintEvent(QPaintEvent *)
{
...
void ZCircleIndicator::drawOuterCircle(QPainter *painter)
{
    painter->save();
...
void ZCircleIndicator::drawInnerCircle(QPainter *painter)
{
    painter->save();
...
void ZCircleIndicator::drawMark(QPainter *painter)
{
    painter->save();
    ...
    for(int i=0;i<=100;i++)
    {
        ...
        if(i%10==0)
        {
            if((value-10)<0)
            {
                strValue=tr("-%1").arg(abs(value-10));
            }else if((value-10)>0)
            {
                strValue=tr("%1").arg(abs(value-10));
            }else{
...
void ZCircleIndicator::drawCoverBall(QPainter *painter)
{
    painter->save();
...
void ZCircleIndicator::drawTextRect(QPainter *painter)
{
    painter->save();
    ...
    //the tips text rect.
    QPointF TipsTopLeftPot(topLeftPot.x()-4,topLeftPot.y()+textRect.height()+4);
    QPointF TipsBottomRightPot(bottomRightPot.x()+4,bottomRightPot.y()+textRect.height()+4);
    QRectF tipsRect(TipsTopLeftPot,TipsBottomRightPot);
    ...
    painter->drawText(tipsRect,Qt::AlignHCenter|Qt::AlignVCenter,this->m_title);
    painter->restore();
}

void ZCircleIndicator::drawCoverCircle(QPainter *painter)
{
    painter->save();
...
void ZCircleIndicator::drawColorPie(QPainter *painter)
{
    painter->save();
...
void ZCircleIndicator::drawIndicator(QPainter *painter)
{
    painter->save();
    ...
    qreal changedAngle=45+increment*(m_currentValue+10);
...
void ZCircleIndicator::resetVariables(QPainter *painter)
{
    m_outerRadius=width()>height() ? height()/2: width()/2;
    m_innerRadius=m_outerRadius/8*7;
    m_coverCircleRadius=m_outerRadius/8*4+10;
...
void ZCircleIndicator::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
...
void ZCircleIndicator::UpdateGraph()
{
    if(!m_bReverse)
    {
        m_currentValue+=0.5;
